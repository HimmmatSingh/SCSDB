...
const TV = () => {
  ...
  const GetTvShows = async () => {
    try {
      const res = await axios.get(
        `https://api.themoviedb.org/3/tv/popular?api_key=2a9337ca88d4c1727b65a3eabff258cb&page=${page}`
      );
  ...
  // Load first page on mount and when page changes
  useEffect(() => {
    GetTvShows();
  }, [page]);
  ...
  return tvShows.length > 0 ? (
    <div className="px-[3%] w-screen min-h-screen overflow-hidden overflow-y-auto">
      ...
      {/* TV Cards */}
      <Cards data={tvShows} title="TV Shows" type="tv" />
